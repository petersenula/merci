declare const Deno: {
  serve: (handler: (req: Request) => Response | Promise<Response>) => void;
};

import { fetchStripeLedgerPaged, saveLedgerItems } from "../ledger_shared.ts";

Deno.serve(async (req) => {
  const url = new URL(req.url);

  // параметры для ручного импорта
  const fromParam = url.searchParams.get("from");
  const toParam = url.searchParams.get("to");

  let fromTs: number;
  let toTs: number;

  if (fromParam && toParam) {
    // 📌 Ручной импорт
    const fromDate = new Date(fromParam + "T00:00:00Z");
    const toDate = new Date(toParam + "T23:59:59Z");

    fromTs = Math.floor(fromDate.getTime() / 1000);
    toTs   = Math.floor(toDate.getTime() / 1000);
  } else {
    // 📌 Крон → rolling window: последние 3 дня
    const now = new Date();

    const fromDate = new Date(
      Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate() - 3)
    );

    const toDate = new Date(
      Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate())
    );

    fromTs = Math.floor(fromDate.getTime() / 1000);
    toTs   = Math.floor(toDate.getTime() / 1000);
  }

  // platform account = null (как было)
  const items = await fetchStripeLedgerPaged(null, fromTs, toTs);
  const count = await saveLedgerItems(null, items);

  return new Response(
    JSON.stringify({
      ok: true,
      count,
      fromTs,
      toTs
    }),
    {
      headers: { "Content-Type": "application/json" },
    }
  );
});
